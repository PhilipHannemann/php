    ...
    function flash_encode($string)
   {
    ...
    function parseRestaurantDetails($content){
        ...
        for ($i=0; $i < count($lines); $i++){
            ...
            if(isTagInLine($lines[$i], "h1")){
                 $place = contentForTag($lines[$i+3], "script");
                 $place = explode("'", $place)[1];
                 $place = str_replace("%DF", "ß" , $place);
                 $place = rawurldecode($place);
                 $place = explode("<p>", $place)[1];
                 $place = str_replace(["<br />", "</p>"], "", $place);
                 $formatted = "";
                 foreach (explode("\n", $place) as $ii) {
                    $ii = trim($ii);
                    if(!empty($ii)){
    ...
    function getWebsites(){
        ...
        return array("http://www.pamir-pizza.de/impressum.php");
    }

    function getAdressPamir(){
        $urls = getWebsites();
        $meals = array();
